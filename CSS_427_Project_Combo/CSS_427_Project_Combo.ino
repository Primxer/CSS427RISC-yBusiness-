...
void loop() {
  ...
  data.batteryLevel = (byte)map(analogRead(batPin), 622, 800, 0, 100);
...
void loop() {
  ...
  if (!autoLevelOn) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("ALO-1");
    data.VR2x_pos = map(constrain(data.yaw-yawZero, -45, 45), -45, 45, 45, 135); //yaw true range is -180 to 180. contrain down to increase sens
